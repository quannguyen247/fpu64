...
module fpu64_mul (
    ...
    always @(*) begin
        ...
        if (sp_nan1 || sp_nan2) begin
            ...
            if (sp_prod[47]) begin
                sp_prod_norm = sp_prod;
                sp_exp = sp_exp + 9'd1;
            end else begin
                sp_prod_norm = sp_prod << 1;
            end
            if ($signed(sp_exp) >= $signed(9'd255)) begin
                ...
                sp_res_exp = 8'd0;
                sp_prod_norm = sp_prod_norm >> (9'd1 - sp_exp);
                sp_guard = sp_prod_norm[23];
                sp_round = sp_prod_norm[22];
                sp_sticky = 1'b0;
                for (i = 0; i < 22; i = i + 1) begin
                    if (sp_prod_norm[i]) sp_sticky = 1'b1;
                end
                sp_round_up = 1'b0;
                case (rm)
                    `RM_RNE: sp_round_up = sp_guard && (sp_round || sp_sticky || sp_prod_norm[24]);
                ...
                endcase
                sp_res_frac = sp_prod_norm[46:24] + (sp_round_up ? 23'd1 : 23'd0);
    ...
    always @(*) begin
        ...
        if (dp_nan1 || dp_nan2) begin
            ...
            if (dp_prod[105]) begin
                dp_prod_norm = dp_prod;
                dp_exp = dp_exp + 12'd1;
            end else begin
                dp_prod_norm = dp_prod << 1;
            end
            if ($signed(dp_exp) >= $signed(12'd2047)) begin
                ...
                dp_res_exp = 11'd0;
                dp_prod_norm = dp_prod_norm >> (12'd1 - dp_exp);
                dp_guard = dp_prod_norm[52];
                dp_round = dp_prod_norm[51];
                dp_sticky = 1'b0;
                for (i = 0; i < 51; i = i + 1) begin
                    if (dp_prod_norm[i]) dp_sticky = 1'b1;
                end
                dp_round_up = 1'b0;
                case (rm)
                    `RM_RNE: dp_round_up = dp_guard && (dp_round || dp_sticky || dp_prod_norm[53]);
                ...
                endcase
                dp_res_frac = dp_prod_norm[104:53] + (dp_round_up ? 52'd1 : 52'd0);
